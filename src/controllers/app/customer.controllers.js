...
const openCustomer=_async(async(req,res)=>{
   ...
   if(_id===-1){
   const result = await  executeQuery(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA =? AND TABLE_NAME=?  AND COLUMN_NAME NOT IN (?);`,['saloon_yourchoice','customers','ShopId']);
